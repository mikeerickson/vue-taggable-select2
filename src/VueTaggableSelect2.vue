<template>
    <div :class="[classes.wrapper]" ref="taggable-select">
        ...
        <select multiple class="bordered hidden" :name="name">
            ...
            >{{getOptionDescription(option)}}</option>
        </select>
        <div v-if="debug">
            {{selectedOptions}}
            <hr />
        ...
        matchingOptions() {
            ...
            if (this.optionLabel && this.optionKey) {
                return this.mutableOptions
                    .filter((option) => this.selectedOptions.findIndex((selected) => selected[this.optionKey] === option[this.optionKey]) < 0)
                    .filter((option) => {
                        return (
                            option[this.optionLabel].toString().toLowerCase().includes(this.searchText.toString().toLowerCase()) ||
                            this.searchText.toString().toLowerCase().includes(option[this.optionKey].toString().toLowerCase())
                        )
                    })
            ...
            if (this.optionLabel) {
                return this.mutableOptions
                    .filter((option) => this.selectedOptions.findIndex((selected) => selected[this.optionLabel] === option[this.optionLabel]) < 0)
                    .filter((option) => option[this.optionLabel].toString().toLowerCase().includes(this.searchText.toString().toLowerCase()))
                    .slice(0, this.maxResults)
            }
            ...
            if (this.optionKey) {
                return this.mutableOptions
                    .filter((option) => this.selectedOptions.findIndex((selected) => selected[this.optionKey] === option[this.optionKey]) < 0)
                    .filter((option) => option[this.optionKey].toString().toLowerCase().includes(this.searchText.toString().toLowerCase()))
                    .slice(0, this.maxResults)
            }

            return this.mutableOptions
                .filter((option) => this.selectedOptions.findIndex((selected) => selected === option) < 0)
                .filter((option) => option.toString().toLowerCase().includes(this.searchText.toString().toLowerCase()))
                .slice(0, this.maxResults)
        },
        isRequired() {
            if (!this.required) {
